  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Three notes scatter in from their exit directions out of the beliefs section ── */}

        {/* Note lined — flew upper-left from beliefs, lands center-left */}
        <motion.div
          className="work-preview__note-lined absolute"
          style={{ left: "calc(50% - 107px)", top: 160, width: 215, zIndex: 25, cursor: "default" }}
          initial={{ x: -280, y: -240, rotate: -25, opacity: 0 }}
          animate={isInView ? { x: 0, y: 0, rotate: -4, opacity: 1 } : {}}
          transition={{ type: "spring", stiffness: 24, damping: 26, mass: 1.3, delay: 0.05 }}
          whileHover={{ scale: 1.04, rotate: -1, zIndex: 40, transition: { duration: 0.25 } }}
        >
          <div style={{
            background: "#FAF8F0",
            ...
            <p className="font-hand" style={{ color: "#1C1814", fontSize: "1.85rem", lineHeight: "32px", margin: 0 }}>
              tirelessly<br />pursue<br />clarity.
            </p>
          </div>
        </motion.div>

        {/* Note grid — flew upper-right from beliefs, lands center-right */}
        <motion.div
          className="work-preview__note-grid absolute"
          style={{ left: "calc(50% + 10px)", top: 200, width: 220, zIndex: 24, cursor: "default" }}
          initial={{ x: 300, y: -200, rotate: 24, opacity: 0 }}
          animate={isInView ? { x: 0, y: 0, rotate: 5, opacity: 1 } : {}}
          transition={{ type: "spring", stiffness: 20, damping: 26, mass: 1.4, delay: 0.16 }}
          whileHover={{ scale: 1.04, rotate: 2, zIndex: 40, transition: { duration: 0.25 } }}
        >
          <div style={{
            background: "#AEACA4",
            ...
            <p style={{ fontFamily: "'Courier New', monospace", color: "#1C1814", fontSize: "1.35rem", lineHeight: 1.45, fontWeight: "bold", margin: 0 }}>
              Software<br />should<br />empower.
            </p>
          </div>
        </motion.div>

        {/* Note plain — flew mostly upward with slight right drift, lands center */}
        <motion.div
          className="work-preview__note-plain absolute"
          style={{ left: "calc(50% - 150px)", top: 240, width: 240, zIndex: 23, cursor: "default" }}
          initial={{ x: 80, y: -320, rotate: -16, opacity: 0 }}
          animate={isInView ? { x: 0, y: 0, rotate: -2, opacity: 1 } : {}}
          transition={{ type: "spring", stiffness: 17, damping: 26, mass: 1.5, delay: 0.3 }}
          whileHover={{ scale: 1.04, rotate: 0, zIndex: 40, transition: { duration: 0.25 } }}
        >
          <div style={{
            background: "#C8B492",
            ...
            <svg width="24" height="22" viewBox="0 0 24 22" fill="none" style={{ marginBottom: 8 }} aria-hidden>
            ...
            <p className="font-hand" style={{ color: "#1C1814", fontSize: "2.2rem", fontStyle: "italic", lineHeight: 1.2, margin: 0 }}>
    ...
    <div className="card-logistics" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="card-logistics__body" style={{ padding: "14px 14px 16px" }}>
        <div className="card-logistics__route" style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: "#111" }}>Buenos Aires</span>
          <span style={{ color: "#888", fontSize: 12 }}>→</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: "#111" }}>Shanghai</span>
        </div>
        <div className="card-logistics__prices" style={{ display: "flex", gap: 10, fontSize: 11, color: "#666", marginBottom: 4 }}>
          <span>Proposed <strong style={{ color: "#111" }}>$308/MT</strong></span>
          <span>Market <strong style={{ color: "#111" }}>$305/MT</strong></span>
        </div>
        ...
        <div className="card-logistics__avatars" style={{ display: "flex", gap: 3, marginBottom: 12 }}>
          {["#C84535", "#4A7FA5", "#5A9B6A"].map((bg, i) => (
            <div key={i} style={{ width: 26, height: 26, borderRadius: "50%", background: bg, border: "2px solid #fff" }} />
          ))}
          <div style={{ width: 26, height: 26, borderRadius: "50%", background: "#ccc", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#555" }}>+2</div>
        </div>
        <div className="card-logistics__contact" style={{ background: "#F8F8F8", borderRadius: 8, padding: "10px 10px", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#ddd", flexShrink: 0 }} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#111" }}>David Andersen</div>
              <div style={{ fontSize: 10, color: "#888" }}>Shipbroker</div>
            </div>
          ...
          <div style={{ fontSize: 10, color: "#777", marginBottom: 5 }}>🌐 Melbourne, AUS (UTC+11)</div>
          <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, color: "#555", marginBottom: 3 }}>
        ...
        <div className="card-logistics__tags" style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
          <span style={{ background: "#FF9800", color: "#fff", fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 3, letterSpacing: 0.5 }}>MEDIUM</span>
          <span style={{ fontSize: 11, color: "#777" }}>Wheat $...</span>
        </div>
        <div style={{ fontSize: 11, color: "#aaa" }}>Buenos Aires → Shanghai</div>
  ...
  return (
    <div className="card-design" style={{ fontFamily: "system-ui, sans-serif", background: "#1A1A1A" }}>
      <div className="card-design__grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, padding: 2 }}>
        {photos.map((p, i) => (
          <div key={i} className="card-design__grid-item" style={{ height: 75, background: p.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: i === 0 ? 18 : 14, fontWeight: 700, color: p.textColor, borderRadius: 4, position: "relative", overflow: "hidden" }}>
            <span>{p.text}</span>
            {i === 2 && <span style={{ position: "absolute", bottom: 6, right: 8, fontSize: 22, fontWeight: 900, opacity: 0.7 }}>V2</span>}
          </div>
        ))}
      </div>
      <div className="card-design__headline" style={{ background: "#111", padding: "8px 12px 4px" }}>
        <div style={{ fontSize: 28, fontWeight: 900, color: "#fff", letterSpacing: -1, lineHeight: 1 }}>W2</div>
      </div>
      <div className="card-design__search" style={{ background: "#1A1A1A", padding: "8px 12px 14px" }}>
        <div style={{ display: "flex", alignItems: "center", background: "#2A2A2A", borderRadius: 20, padding: "6px 14px", gap: 8 }}>
          <span style={{ flex: 1, fontSize: 12, color: "#888" }}>Search...</span>
    ...
    <div className="card-race" style={{ fontFamily: "system-ui, sans-serif", background: "#0A1A0F" }}>
      <div className="card-race__map" style={{ height: 150, background: "linear-gradient(145deg, #0D2B15 0%, #1A3D22 40%, #0F2210 100%)", position: "relative", overflow: "hidden" }}>
        <svg className="card-race__map-svg" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} viewBox="0 0 275 150" fill="none" preserveAspectRatio="xMidYMid slice">
          <path d="M 20 130 C 40 110, 60 90, 90 80 C 120 70, 140 95, 165 75 C 190 55, 210 40, 245 35" stroke="#4ADE80" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="4 2" />
          <circle cx="245" cy="35" r="6" fill="#4ADE80" fillOpacity="0.8" />
          <circle cx="245" cy="35" r="12" stroke="#4ADE80" strokeOpacity="0.3" strokeWidth="1.5" />
          <path d="M 0 120 C 30 115, 60 125, 90 118 C 120 111, 150 120, 180 115 C 210 110, 240 118, 275 115" stroke="#1E4D28" strokeWidth="1" />
          <path d="M 0 140 C 50 135, 100 142, 150 137 C 200 132, 240 140, 275 136" stroke="#1E4D28" strokeWidth="1" />
        </svg>
      </div>
      <div className="card-race__photos" style={{ display: "flex", gap: 2, padding: "2px 2px 0" }}>
        <div style={{ flex: 1, height: 58, background: "#2A4030", borderRadius: "0 0 0 4px", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ flex: 1, height: 58, background: "#1E3028", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
      ...
      <div className="card-race__timeline" style={{ padding: "10px 14px 12px", background: "#0A1A0F" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: 10, color: "#4ADE80", fontWeight: 600 }}>00:55</span>
          <div className="card-race__progress" style={{ flex: 1, height: 3, background: "#1E3028", borderRadius: 2, position: "relative" }}>
            <div style={{ position: "absolute", left: 0, top: 0, width: "60%", height: "100%", background: "#4ADE80", borderRadius: 2 }} />
          </div>
          <span style={{ fontSize: 10, color: "#4ADE80", fontWeight: 600 }}>17:23</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="card-payment" style={{ fontFamily: "system-ui, sans-serif", padding: "16px 16px 18px" }}>
      <div className="card-payment__header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 12, color: "#888", cursor: "pointer" }}>Cancel</span>
        <div style={{ background: "#F5F5F5", borderRadius: 8, padding: "4px 10px" }}>
          <div style={{ fontSize: 10, color: "#888", marginBottom: 1 }}>Order summary</div>
          <div style={{ fontSize: 9, color: "#555" }}>1 Items</div>
        </div>
      </div>
      <div className="card-payment__total" style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 10, color: "#888", marginBottom: 2 }}>Total due</div>
        <div style={{ fontSize: 26, fontWeight: 800, color: "#C84535", letterSpacing: -1 }}>R6,699.86</div>
      </div>
      <div className="card-payment__pay-options" style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: "#aaa", marginBottom: 6 }}>Pay with</div>
        <div className="card-payment__apple-pay" style={{ background: "#111", borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 13, color: "#fff", fontWeight: 600 }}>Apple Pay</span>
        </div>
        <div className="card-payment__options">
          {["Credit / debit card", "TFG Money account", "DFT / Capfec Pay", "WaveTyne"].map((opt, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #f0f0f0" }}>
              <span style={{ fontSize: 10, color: "#555" }}>{opt}</span>
              <div style={{ display: "flex", gap: 3 }}>
                {i === 0 && ["#1A1AE0", "#E01A1A"].map((c, j) => <div key={j} style={{ width: 16, height: 10, borderRadius: 2, background: c }} />)}
                {i > 0 && <div style={{ width: 24, height: 12, borderRadius: 3, background: "#e8e8e8" }} />}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="card-payment__summary" style={{ background: "#F8F8F8", borderRadius: 6, padding: "8px 10px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#C84535", marginBottom: 2 }}>
          <span>Discount</span><span>-R600.00</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, fontWeight: 700, color: "#111" }}>
